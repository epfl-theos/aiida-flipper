...
def delete_calcs_rec(master_pk, dry_run, force):
    ...
    qb = QueryBuilder().append(ChillstepCalculation, filters={'id':master_pk}).append(
        Calculation, tag='c', project='id').order_by({'c':{'ctime':{'order':'desc'}}})
...
def main_delete_chillsteps(pks,  dry_run=False, force=False):
    ...
    pks_to_delete = QueryBuilder().append(ChillstepCalculation, project='id', filters={'id':{'in':pks}}, tag='cs').all()
