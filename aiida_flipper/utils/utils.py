...
def get_pseudos(
    calc=None,
    pseudo_family_name=None,
    structure=None,
    with_pseudo_in_label=False,
    ...
    **kwargs
    ...
    # eg. Li=MyPseudo
    for kn, upfdata in six.iteritems(kwargs):
        assert isinstance(upfdata, UpfData), '{} is not an instance of {}'.format(upfdata, UpfData)
        if kn not in list(pseudo_dict.keys()):
            print('WARNING: {} is not in the structure'.format(kn))
        pseudo_dict[kn] = upfdata
    if return_cutoffs:
        ecutwfc, ecutrho = get_suggested_cutoff(pseudo_family_name, list(pseudo_dict.values()), min_ecutwfc=min_ecutwfc)
...
def get_suggested_cutoff(pseudo_family_name, pseudos, min_ecutwfc=0):
    ...
    try:
        ...
        raise AttributeError('raised when asking for cutoff and duality\n' 'for pseudos {}\n' '{}'.format(pseudos, e))
    return ecutwfc, ecutrho
