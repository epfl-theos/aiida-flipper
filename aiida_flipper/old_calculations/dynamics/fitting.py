...
def get_pinball_factors_inline(parameters, trajectory_scf, trajectory_pb):
    ...
    coefs, mae = fit_with_lin_reg(
        forces_scf, forces_pb, verbosity=0, divide_r2=params_dict['divide_r2'], signal_indices=signal_indices
    )
    ...
    return {
        ...
        ParameterData(
            dict=dict(
                coefs=coefs.tolist(), mae=mae_f, nr_of_coefs=len(coefs), indices_removed=sorted(starting_indices)
            )
        )
    }
...
def get_structures_from_trajectories_inline(parameters, label='', description='', **branches):
    ...
    sorted_trajectories = list(zip(*sorted(branches.items())))[1]
    ...
    deltasteps = int(totsteps / nr_of_configurations)  # jump between steps
    structures = {}
...
def get_configurations_from_trajectories_inline(parameters, structure, **branches):
    ...
    if 'indices_to_overwrite' in parameters_d:
        ...
        if isinstance(symbols, (set, tuple, list)):
            pass
        elif isinstance(symbols, six.string_types):
            symbols = [symbols]
        else:
    ...
    new_positions = np.repeat(np.array([positions]), nr_of_configurations, axis=0)
    time_split = int((sorted_positions.shape[0] - 1) / (nr_of_configurations - 1))
    # time_split:
    ...
    for idx in range(nr_of_configurations):
        if remap:
            frac_coords = get_frac_coords(
                sorted_positions[time_indices[idx], indices_to_read, :], sorted_cells[time_indices[idx]]
            )
...
class FittingCalculation(ChillstepCalculation):
    ...
    def launch_calculations(self):
        ...
        for card, key in (('SYSTEM', 'tot_charge'), ('CONTROL', 'max_seconds'), ('ELECTRONS', 'conv_thr'),
                          ('ELECTRONS', 'electron_maxstep'), ('ELECTRONS',
                                                              'mixing_beta'), ('ELECTRONS', 'diagonalization')):
            try:
    ...
    def launch_replays(self):
        ...
        inputs_dft = dict(
            moldyn_parameters=get_or_create_parameters(
                dict(
                    nstep=self.ctx.nstep,
        ...
        for s in own_inputs['structure'].get_site_kindnames():
            ...
            pseudos_dft['pseudo_{}'.format(s)
                       ] = own_inputs.get('pseudo_{}_dft'.format(s), None) or own_inputs['pseudo_{}'.format(s)]
            pseudos_flipper['pseudo_{}'.format(s)
                           ] = own_inputs.get('pseudo_{}_flipper'.format(s), None) or own_inputs['pseudo_{}'.format(s)]
    ...
    def fit(self):
        ...
        for traj in (trajectory_scf, trajectory_pb):
            ...
            if shape[0] != nstep:
                ...
                raise Exception(
                    'Wrong shape of array returned by {} ({} vs {})'.format(traj.inp.total_trajectory.id, shape, nstep)
                )

        # IMPORTANT TODO: Exclude forces where scf failed! The hustler (maybe?) doesn't fail if SCF doesn't converge...

        params_d = get_or_create_parameters(
            dict(
        ...
        calc, res = get_pinball_factors_inline(
            parameters=params_d, trajectory_scf=trajectory_scf, trajectory_pb=trajectory_pb
        )
...
class FittingFlipper1RandomlyDisplacedPosCalculation(FittingCalculation):
    ...
    def start(self):
        ...
        nr_of_configurations = int(
            float(parameters_d['nr_of_force_components']) / nr_of_pinballs / 3
        ) + 1  # Every pinball atoms has 3 force components
        ...
        c, res = rattle_randomly_structure_inline(
            structure=self.inp.structure, parameters=get_or_create_parameters(rattling_parameters_d)
        )
