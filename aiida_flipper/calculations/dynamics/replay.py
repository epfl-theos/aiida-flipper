...
class ReplayCalculation(ChillstepCalculation):
    ...
    def run_calculation(self):
        ...
        for linkname, input_node in six.iteritems(self.get_inputs_dict()):
        ...
        if moldyn_parameters_d.get('is_hustler', False):
            ...
            kwargs = dict(
                trajectory=lastcalc.out.output_trajectory,
    ...
    def evaluate_calc(self):
        ...
        if lastcalc.get_state() != calc_states.FINISHED:
            ...
            if backoff_counter > NTRIALS:
                raise Exception(
                    'My last calculation {} did not finish, and I used my {} trials up!'.format(lastcalc, NTRIALS)
                )
            try:
            ...
            try:
                ...
                pass
            total_energy_max_fluctuation = self.inputs.moldyn_parameters.get_dict().get(
            ...
            if total_energy_max_fluctuation is not None:
                ...
                if diff > total_energy_max_fluctuation:
                    print('NOOO')
                    raise Exception(
                        'Fluctuations of the total energy ( {} ) exceeded threshold ( {} ) !'.format(
                            diff, total_energy_max_fluctuation
                        )
                    )
                else:
                    print('ALL GOOD')
    ...
    def produce_output_trajectory(self):
        ...
        if len(d) > 1:
            ...
            returnval = {'total_trajectory': list(d.values())[0]}
        else:
    ...
    def get_slave_calculations(self):
        ...
        return list(zip(*sorted_calcs))[1]
    ...
    def get_output_trajectory(self, store=False):
        ...
        qb.append(
            ...
            output_of='m',
            edge_project='label',
            filters={'state': calc_states.FINISHED},
            edge_filters={
                'type': LinkType.CALL.value,
                'label': {
                    'like': 'calc_%'
